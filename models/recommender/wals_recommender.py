...
class WALSRecommender(Recommender):
    ...
    # AF(model, m, n, k, c, num_new_users) = a matrix factorization collaborative filtering recommendation model
    #   of embedding dimension k with m entity embeddings model.user_factors
    #   and n item embeddings model.item_factors.  The model weights unobserved entries by factor c.
    ...
    #   - k and c are private and immutable
    #   - model is never returned

    def __init__(self, m: int, n: int, k: int = 10, c: float = 0.1):
        ...
        Args:
            ...
            c: weight for unobserved entries.  Requires 0 < c <= 1
        """
        assert m > 0
        assert n > 0
        assert k > 0
